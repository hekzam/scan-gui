...
public:
    ...
    PageInfo(QString const& pageName, int pageInJSON) : m_pageName(pageName), m_pageInJSON(pageInJSON), m_fieldNum(0), m_pageInFiles(0){};
...
public:
    ...
    CopyInfo(QString const& copyName, int copyInJSON) : m_copyName(copyName), m_copyInJSON(copyInJSON), m_numPages(0), m_copyInFiles(0){};
    ...
    QStringList getPagesPathList(){
        QStringList paths;
        for (auto itPage = m_copyPageMap.begin(); itPage != m_copyPageMap.end(); itPage++)
...
public:
    ...
    SubjectInfo(QString const& subjectName, mJSON::dataCopieJSON *data = nullptr) : m_subjectName(subjectName), m_data(data), m_numCopies(0){};
    ...
    void addPageToCopy(QString const& copyName, QString const& pageName, int pageInJSON){
        if (containsCopy(copyName))
            m_subjectCopyMap[copyName].addPage(pageName, pageInJSON);
    }

    void addFieldToCopyPage(QString const& copyName, QString const& pageName, QString const& fieldName){
        if (containsCopy(copyName))
    ...
    QStringList getCopiesPathList(){
        QStringList paths;
        for (auto itCopy = m_subjectCopyMap.begin(); itCopy != m_subjectCopyMap.end(); itCopy++){
            paths.append(itCopy->second.getPagesPathList());
        }
        return paths;
    }

    void setPagePath(QString const& copyName, QString const& pageName, QString const& path){
        if (containsCopy(copyName))
    ...
    void setCopyPageInFiles(QString const& copyName, QString const& pageName, int copyInFiles){
        if (containsCopy(copyName))
