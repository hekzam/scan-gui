...
void Boundary::init()
{
  ...
  connect(menuButton, &QPushButton::clicked, this,
          &Boundary::ouvrirExplorateurFichier);
...
void Boundary::initTableFilter()
{
  ...
  connect(syntax, &QCheckBox::stateChanged, sortTable,
          &SortTable::editSyntaxColumn);

  QCheckBox *semantic = new QCheckBox("Semantic", checkBoxGroup);
  semantic->setCheckState(Qt::Checked);
  connect(semantic, &QCheckBox::stateChanged, sortTable,
          &SortTable::editSemanticColumn);

  QCheckBox *metric1 = new QCheckBox("Metric 1", checkBoxGroup);
  connect(metric1, &QCheckBox::stateChanged, sortTable,
          &SortTable::editMetric1Column);

  QCheckBox *metric2 = new QCheckBox("Metric 2", checkBoxGroup);
  connect(metric2, &QCheckBox::stateChanged, sortTable,
          &SortTable::editMetric2Column);
...
void Boundary::initTableView(QStringList const &listeFichiers)
{
  ...
  connect(checkBoxButton, &QPushButton::clicked, this,
          &Boundary::displayTableFilter);
